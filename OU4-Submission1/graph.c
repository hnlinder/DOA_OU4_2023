...
struct graph
{
        ...
        dlist *nodes;   // List containing nodes 
};
...
graph *graph_insert_node(graph *g, const char *s)
{
        ...
        // Set name
        Node->identifier = s;
        // Create list of known neighbours, i.e. an empty list
        Node->neighbours = dlist_empty(NULL);
        // Set seen status
        Node->seen_status = false;
        // Inserting node into the graph
...
graph *graph_delete_node(graph *g, node *n)
{
        ...
        while (!dlist_is_end(g->nodes, pos))
        {
                ...
                {
                        // ...remove node from graph and free up memory.
...
{
        // Extract and delete nodes from list 
        // Corresponding memory is free'd by 'graph_delete_node'
