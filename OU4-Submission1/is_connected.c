...
/*
* **Assignment 4** in the course "Datastructures and algorithms" 
* at the Department of Computing Science, Umea University.
...
int first_non_white_space(const char *s) 
{
        ...
        int i = 0;
        // Continue until non-whitespace found or EOL 
...
int last_non_white_space(const char *s)
{
        ...
        // Starting at last character
        int i = strlen(s) - 1;
        // Continue moving to start of string
        // Stop when non-whitespace found
...
{       
        ...
        // Start at first non-whitespace
        int i = beg;
        int count = 1;
        int check = 0;
        // Move towards last non-whitespace
        while (i <= end)
        {
...
/**
* white_space() - Tests if a given line of string is exclusively "space"
* @s: Pointer to character.
*
...
char *trim(char *line)
{
        ...
        // Adds an "ending" to string
        s[length] = '\0';
        // Free's memory of 'stemp'
        free(stemp);
        return s;
}
...
graph *build_graph(char **str1, char **str2, int max_elements, int edges)
{
        ...
        for (int i = 0; i < edges; i++)
        {
                ...
                // Insert nodes with corresponding names only if they are not 
...
/**
* find_path() - function that utilizes breadth-first search
* @g: Pointer to graph.
* @src: Pointer to source-node.
* @dest: Pointer to destination-node
*
*Returns: 1 if destination node is found
*         0 if not.
*/
bool find_path(graph *g, node *src, node *dest)
{
        ...
        {       // Inspects the node on the queues front
                ...
                if (nodes_are_equal(inspectedNode, dest))
                {
                        ...
                        // Reset seen status of all nodes
                        g = graph_reset_seen(g);
                        // Kills the queue and frees the allocated memory
                        queue_kill(q);
                        return true;
                }
